...
int		print_p(t_frmt frmt, va_list ap)
{
	...
	else if (frmt.sharp || frmt.plus || frmt.zero || frmt.space)
	{
		if ((frmt.min <= (frmt.max ? frmt.max : count_ubase(u, 16)) || !frmt.min || frmt.zero) && ((ox = 0) == 0))
			bytes += ft_putstr("0x");
		bytes += repeat_char((frmt.zero && (frmt.min < (frmt.max ? frmt.max : count_ubase(u, 16)) || !frmt.max)) 
			? '0' : ' ', frmt.min - (frmt.max ? frmt.max : count_ubase(u, 16)) - 2);
...
// 		return (bytes + repeat_char(' ', frmt.min - bytes));
// 	}
// 	if (frmt.sharp || frmt.plus || frmt.zero || frmt.space)
...
// 	n = (frmt.max && frmt.max > count_ubase(u, 16)) ? frmt.max + 2: count_ubase(u, 16) + 2;
// 	if ((frmt.min <= n || !frmt.min) && ((ox = 0) == 0))
// 		bytes += ft_putstr("0x");
// 	bytes += repeat_char(frmt.zero && (frmt.min < n || !frmt.max)
// 		? '0' : ' ', frmt.min - n);
// 	bytes += ft_putstr((ox) ? "0x" : NULL);
// 	if (frmt.max > count_ubase(u, 16))
// 		bytes += repeat_char('0', frmt.max - count_ubase(u, 16));
// 	return (bytes + ((!u && frmt.prec && !frmt.max) ?
// 		0 : print_ubase(u, 16, 0, 0)));
...
int		print_uox(t_frmt frmt, uintmax_t u)
{
	...
	if (frmt.minus)
	{
		...
		bytes += (ox == 2) ? ft_putstr((frmt.modifier == 'x'
			? "0x" : "0X")) : 0;
	...
	{
		if (frmt.min <= ((frmt.max && frmt.max > count_ubase(u, 16)) ? frmt.max : count_ubase(u, base)) || frmt.zero)
		{
		...
		bytes += repeat_char((frmt.zero && (frmt.min < ((frmt.max && frmt.max > count_ubase(u, 16)) ? frmt.max : count_ubase(u, base)) || !frmt.max)) ? '0' : ' ', frmt.min - ((frmt.max && frmt.max > count_ubase(u, 16)) ? frmt.max : count_ubase(u, base)) - (frmt.sharp && base == 16 ? 2 : 0) - (frmt.sharp && base == 8 ? 1 : 0));
		if (ox && u != 0)
			bytes += (ox == 2) ? ft_putstr((frmt.modifier == 'x' ? "0x" : "0X")) : ft_putchar('0');
		if (frmt.max > count_ubase(u, base))
			bytes += repeat_char('0', frmt.max - count_ubase(u, base) - (frmt.sharp && base == 8 ? 1 : 0));
